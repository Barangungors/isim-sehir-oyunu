...
const els = {
    ...
    voteStatusList: document.getElementById('vote-status-list'), votingTimer: document.getElementById('voting-timer'), voteCategory: document.getElementById('vote-category'), voteAvatar: document.getElementById('vote-avatar'), votePlayerName: document.getElementById('vote-player-name'), voteAnswer: document.getElementById('vote-answer'), countUp: document.getElementById('count-up'), countDown: document.getElementById('count-down'), btnVoteYes: document.getElementById('btn-vote-yes'), btnVoteNo: document.getElementById('btn-vote-no'), voteResultOverlay: document.getElementById('vote-result-overlay'), voteResultText: document.getElementById('vote-result-text'), resultsDetailsContainer: document.getElementById('results-details-container')
};
...
socket.on('newVotingItem', ({ item, timer, autoVoteId }) => {
    ...
    void els.voteAnswer.offsetWidth;
    ...
    els.countUp.innerText = "1 OY"; els.countDown.innerText = "0 OY"; 
    
    // YENİ: İlk otomatik oyu kutu olarak ekle
    const iconsContainer = document.getElementById('vote-icons-container');
    if(iconsContainer) {
        iconsContainer.innerHTML = `<div class="w-10 h-10 flex items-center justify-center rounded-lg bg-emerald-500/20 border border-emerald-500 text-emerald-400 text-lg shadow-[0_0_10px_rgba(16,185,129,0.2)] animate-fade-in-up"><i class="fa-solid fa-check"></i></div>`;
    }
...
socket.on('voteUpdate', ({ playerId, vote }) => {
    ...
    if (li) {
        ...
        if (vote === 1) { 
            li.classList.add('border-emerald-500/50', 'bg-emerald-500/10'); 
            li.querySelector('.status-icon').innerHTML = '<i class="fa-solid fa-check text-emerald-400 text-xl animate-bounce"></i>'; 
            els.countUp.innerText = (parseInt(els.countUp.innerText) + 1) + " OY";
        } else { 
            li.classList.add('border-rose-500/50', 'bg-rose-500/10'); 
            li.querySelector('.status-icon').innerHTML = '<i class="fa-solid fa-xmark text-rose-400 text-xl animate-bounce"></i>'; 
            els.countDown.innerText = (parseInt(els.countDown.innerText) + 1) + " OY"; 
        }

        // YENİ: Ortadaki dizilime kutu ekle
        const iconsContainer = document.getElementById('vote-icons-container');
        if(iconsContainer) {
            const box = document.createElement('div');
            box.className = "w-10 h-10 flex items-center justify-center rounded-lg text-lg animate-fade-in-up shadow-lg ";
            if(vote === 1) {
                box.className += "bg-emerald-500/20 border border-emerald-500 text-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.2)]";
                box.innerHTML = '<i class="fa-solid fa-check"></i>';
            } else {
                box.className += "bg-rose-500/20 border border-rose-500 text-rose-400 shadow-[0_0_10px_rgba(244,63,94,0.2)]";
